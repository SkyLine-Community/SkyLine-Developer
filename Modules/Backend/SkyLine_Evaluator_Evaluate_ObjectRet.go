...
func applyFunction(Env *Environment_of_environment, fn Object, args []Object) Object {
	...
	default:
		...
		return NewError(Map_Eval[ERROR_INVALID_FUNCTION_NOT_FOUND_OR_UNSUPPORTED_TYPE](string(fn.Inspect())).Message)
	}
}
...
func evalIndexExpression(left, index Object) Object {
	...
	default:
		return NewError(Map_Eval[ERROR_INVALID_INDEX_EXPRESSION_OR_UNSUPPORTED_INDEX](string(left.Type_Object())).Message)
	}
}
...
func evalHashLiteral(node *HashLiteral, Env *Environment_of_environment) Object {
	...
	for keyNode, valueNode := range node.Pairs {
		...
		if !ok {
			return NewError(Map_Eval[ERROR_INVALID_HASH_KEY_COULD_NOT_PARSE_HASHKEY](string(key.Type_Object())).Message)
		}
...
func evalHashIndexExpression(left, index Object) Object {
	...
	if !ok {
		return NewError(Map_Eval[ERROR_INVALID_HASH_KEY_COULD_NOT_BE_USED_AS_KEY](string(index.Type_Object())).Message)
	}
...
func EvalRegisterCall(val Object) Object {
	switch v := val.(type) {
	case *String:
		// Continue to parse the next statement or library
		if ok := StandardLibNames[val.Inspect()]; ok {
			RegisterStandard[val.Inspect()]()
		}
	default:
		return NewError("Sorry but this type is unsupported, MUST be a string in call to register() -> got %s", v.Type_Object())
	}
	return &Nil{}
}
...
func evalInfixExpression(operator string, left, right Object) Object {
	...
	case left.Type_Object() != right.Type_Object():
		return NewError(Map_Eval[ERROR_INVALID_DATATYPE_INIFX_OPERATION_LR](string(left.Type_Object()), string(right.Type_Object()), operator).Message)
	default:
		return NewError(Map_Eval[ERROR_INVALID_OPERATOR_DURING_EVALUATION](operator, string(left.Type_Object()), string(right.Type_Object())).Message)
	}
}
...
func evalIntegerInfixExpression(operator string, left, right Object) Object {
	...
	default:
		...
		return NewError(fmt.Sprintf("Integer Infix Expression does not exist %s and %s", string(left.Type_Object()), string(right.Type_Object())))
	}
}
...
func evalFloatInfixExpression(operator string, left, right Object) Object {
	...
	case *Float:
		leftVal = left.Value
	default:
		return NewError(fmt.Sprintf("Please make sure the type of the left value is equal to the right type value in -> %s %s %s", operator, string(left.Type_Object()), string(right.Type_Object())))
	}
	...
	case "!=":
		return nativeBoolToBooleanObject(leftVal != rightVal)
	default:
		return NewError(fmt.Sprintf("Make sure the operator you are using is supported for float infix expressions in -> (%s %s %s)", operator, string(left.Type_Object()), string(right.Type_Object())))
	}
}
...
func evalStringInfixExpression(operator string, left, right Object) Object {
	...
	case "+=":
		return &String{Value: leftVal + rightVal}
	default:
		return NewError(fmt.Sprintf("Make sure the operator you used is support for string infix expressiong, you gave an unknown operator in -> (%s %s %s)", operator, string(left.Type_Object()), string(right.Type_Object())))
	}
}
