...
func (File *FileCurrentWithinParserEnvironment) VerifyFileExists(logging bool) bool {
	if File.Filename == "" {
		fmt.Println(Map_Parser[ERROR_FILE_INTEGRITY_IS_EMPTY]())
		os.Exit(0)
	}
	...
	if x == os.ErrNotExist {
		if logging {
			fmt.Println(Map_Parser[ERROR_FILE_INTEGRITY_DOES_NOT_EXIST](FileCurrent.Get_Name()))
		}
		return false
	}
	...
	if f.IsDir() {
		File.IsDir = true
		if logging {
			fmt.Println(Map_Parser[ERROR_FILE_INTEGRITY_IS_DIRECTORY](FileCurrent.Get_Name()))
		}
		return false
	}
	return true
...
func (File *FileCurrentWithinParserEnvironment) Verify_CSC(logging bool) bool {
	if ext := filepath.Ext(File.Get_Name()); ext == ".csc" {
		...
		if logging {
			fmt.Println(Map_Parser[ERROR_FILE_INTEGRITY_BAD_FILE](FileCurrent.Get_Name()))
		}
		return false
	}
}
