...
// Metadata will grab the metadata of the image and print it out
// Represented as forensics.png.meta()
func (META *PNG_Meta) Metadata(byter *bytes.Reader) {
...
func (SessionSets *Settings) Settings_Inject_New(key, ofile, infile, filemode, decode, encode, offset, payload, chunktoinject string) {
	...
	/*
		fmt.Println("Filled out settings - ")
		fmt.Println("Decode  ? ", SessionSets.Decode)
		fmt.Println("Encode  ? ", SessionSets.Encode)
		fmt.Println("FlMode  ? ", SessionSets.FileMode)
		fmt.Println("Offset  ? ", SessionSets.ImageOffset)
		fmt.Println("IChunk  ? ", SessionSets.InjectableChunk)
		fmt.Println("Ifile   ? ", SessionSets.InputFile)
		fmt.Println("Key     ? ", SessionSets.Key)
		fmt.Println("Payload ? ", SessionSets.Payload)
		fmt.Println("Ofile   ? ", SessionSets.OutputFile)
	*/
}
...
func (SessionSets *Settings) Injection_Standard_Payload(reader *bytes.Reader) bool {
	...
	if SessionSets.ImageOffset != 0x00 && SessionSets.ImageOffset != 0 && SessionSets.Payload != "" {
		// Inject
		var metac PNG_Meta
		...
		//fmt.Printf("Payload Original: % X\n", []byte(SessionSets.Payload))
		//fmt.Printf("Payload: % X\n", metac.Chunk.FD)
		return Injection_Writer("outout.png", SessionSets.ImageOffset, false, 0777, reader, binmarshalbytes)
	} else {
